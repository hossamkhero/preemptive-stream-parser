...
Installs engine + markdown + json from ${DEFAULT_REPO}.
...
Examples:
  curl -fsSL https://raw.githubusercontent.com/${DEFAULT_REPO}/master/scripts/install.sh | bash
  curl -fsSL https://raw.githubusercontent.com/${DEFAULT_REPO}/master/scripts/install.sh | bash -s -- --to src/stream-parser --ref v0.1.0 --with-tests
USAGE
}
...
cp -R "$src/core/markdown" "$TARGET/"
cp -R "$src/core/json" "$TARGET/"
cp "$src/core/index.ts" "$TARGET/index.ts"

if [[ "$WITH_TESTS" -eq 1 ]]; then
  mkdir -p "$TARGET/__tests__"
  cp "$src/core/__tests__/MDStreamParser.test.ts" "$TARGET/__tests__/"
