...
Installs engine + markdown from ${DEFAULT_REPO}.
...
Examples:
  curl -fsSL https://raw.githubusercontent.com/${DEFAULT_REPO}/master/scripts/install-md.sh | bash
  curl -fsSL https://raw.githubusercontent.com/${DEFAULT_REPO}/master/scripts/install-md.sh | bash -s -- --to src/stream-parser --ref v0.1.0 --with-tests
USAGE
}
